...
function activate(_) {
	...
	if (shouldInitCodeLens) selectionFct('load codelens')
    ...
    try {
      return new Promise((resolve) => exec(`cd ${cwd} && npm install`, async error => {
        if (progress) progress.emit('finish');
  ...
  vscode.commands.registerCommand('blackbox.comment.add.suggestion', async ({ range, previousRange = [] }) => {
	...
	var content = editor.document.getText(selection)
  ...
  function formatTerminalContent(content) {
						...
						if (is_stream && is_excess_context_size == false && webSocketChat.readyState == 1 && is_demo == false){// Stream Response
							webSocketChat.onmessage = (event) => {
								...
								if (event.data == 'stream done'){
									...
									requestSuggestions({command: 'generate', allMessages: data.allMessages, userId: userId})
	...
	async function requestSuggestions(data) {
		...
		try{
			...
			if (chatPanel) chatPanel.webview.postMessage({ command: "showFollowUp", result })
		}catch(e){
		...
		return `
			...
			<body>
				<div class="chat-connected active">
					...
					<div class="chat-footer">
						...
						<div class="switch-container">
							<div class="switch-title" >Include Active View Context</div>
							<label class="switch">
				...
				(function() {
					...
					function showFollowUpSuggestions(suggestionsArr) {
						var suggestions = ""
						suggestionsArr.forEach((el, idx)=>{
							suggestions += '<div class="code-block-input chatMessage sent suggestion-text"><div class="chatMessage-text">'+el+'</div></div>'
						})
						const $suggestions = $('<div class="starter-suggestions-holder"><div class="suggestion-title">Here are some follow-up suggestions</div><div class="suggestion-container"><div class="suggestions-wrapper">'+suggestions+'</div></div></div>')
						$(".chat-area").append($suggestions);
					...
					window.addEventListener('message', event => {
						...
						if(data.command === "showAnswer"){
							...
							showFollowUpSuggestions(data.result);
						}
					});
  ...
  function getFolderTimestamp(folderName) {
	...
	function getActiveLines() {
		const editor = vscode.window.activeTextEditor || activeEditor;
